...
class EspProvisioningParams {
    ...
    private static final boolean DEBUG = false; // Set to false for release, true for debug
    ...
    private void addDataFor6Bytes(byte[] buf, int sequence, int seqCrc, boolean tailIsCrc) {
        if (DEBUG) {
            Log.w(TAG, "buf=" + Arrays.toString(buf) + " , seq=" + sequence + " , seqCrc=" + seqCrc + " , tailIsCrc=" + tailIsCrc);
        }
